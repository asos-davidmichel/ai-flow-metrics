...
function buildSystemPrompt(board: Board, boardDir: string): string {
    ...
    return [
        ...
        `Proactive verification rule: when you identify a pattern that could be confirmed with available data — e.g. "items with no assignee", "items stuck in New", "long-waiting items" — do NOT hand it back to the user as something to check manually. Instead, load work_items.json (or fetch_live_work_items) immediately and answer it yourself. List the specific item IDs and titles. Only say "the data can't tell you" if the information is genuinely absent from all available sources (e.g. ticket description quality, team conversation content).`,
