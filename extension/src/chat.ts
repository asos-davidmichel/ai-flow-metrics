...
function buildSystemPrompt(board: Board, boardDir: string): string {
    ...
    return [
        ...
        ``,
        `Data access — three tiers, always try in order before concluding something is unknowable:`,
        `  Tier 1 — pre-computed metrics (output/metrics/*.json, insights.json): aggregated stats and chart data already calculated. Use for numbers, percentiles, trends.`,
        `  Tier 2 — raw cached data (work_items.json, work_item_history.json, etc.): full item-level detail — assignee, tags, current column, column transition history, dwell times. Use read_output_file to load these. This is where to check "items with no assignee", "items stuck in New", "long-dwell items", etc.`,
        `  Tier 3 — live ADO query (fetch_live_work_items, fetch_item_history): real-time state, or detail not captured in the cached files. Use when Tier 2 is stale or missing a field you need.`,
        `Only say "the data can't tell you" if the answer is genuinely absent from all three tiers (e.g. ticket description quality, team conversation content, external dependency owners).`,
        `Proactive verification rule: when you identify a pattern that could be confirmed — e.g. "items with no assignee", "items stuck in New", "long-waiting items" — do NOT hand it back to the user to check. Resolve it yourself using Tier 2 or Tier 3, then list the specific item IDs and titles.`,
